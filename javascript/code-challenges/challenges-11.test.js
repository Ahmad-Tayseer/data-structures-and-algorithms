...
const comparing = (data, num) => {
  const equalArr = data.filter(element => element === num);
  const equalTimes = equalArr.length;
  return equalTimes;
};

const count = (target, input) => {
  const array = input.map(element => comparing(element, target));
  const equalTimes = array.reduce((prev, curr) => prev + curr, 0);
  return equalTimes;
  // Solution code here...
};
...
------------------------------------------------------------------------------------------------ */
const sum = (data) => {
  const summation = data.reduce((prev, curr) => prev + curr, 0);
  return summation;
};

const totalSum = (input) => {
  const sumArray = input.map(element => sum(element));
...
const modify = (data) => {
  const filterArray = data.filter(element => typeof element === 'number' && element % 5 === 0);
  const powerArray = filterArray.map(element => Math.pow(2, element));
  return powerArray;
};

const divisibleByFiveTwoToThePower = (input) => {
  const finalArray = input.map(element => modify(element));
  return finalArray;
  // Solution code here...
};
...
let findMaleAndFemale = (data) => {
  const filteredArr = data.filter(element => element.gender === 'male' || element.gender === 'female');
  const namesStr = filteredArr.map(element => element['name']).join(' and ');
  return namesStr;
  // Solution code here...
};
...
let findShortest = (data) => {
  const shortest = data.reduce((prev, curr) => Number(prev.height) < Number(curr.height) ? prev : curr);
