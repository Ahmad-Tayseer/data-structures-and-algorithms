...
const sortByChildren = (charArray) => {
  return charArray.sort((element1, element2) => {
    if (element1.children.length < element2.children.length) {
      return -1;
    } else if (element1.children.length === element2.children.length) {
...
const isCapitalized = (str) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    let result = /^[A-Z]/.test(arr[i]);
    if (result === true) {
      array.push(arr[i].match(/[A-Z]/ig).join(''));
    }
  }
  return array;
  // Solution code here...
...
const matchMonth = (input) => {
  // let result = input.match(/October/) || input.match(/october/) || input.match(/Oct/) || input.match(/oct/);
  let arr = [/October/, /october/, /Oct/, /oct/];
  let result = false;
  for (let i = 0; i < arr.length; i++) {
    if (input.match(arr[i]) === null) {
      result = result || false;
    } else {
      result = true;
    }
