...
const longestString = (arr) => {
  let positionIndex = 0;
  if (arr.length > 0) {
    let indexStr = arr.reduce((prev, curr) => {
      return prev.length > curr.length ? prev : curr;
    });
    positionIndex = arr.indexOf(indexStr);
  }
  else {
    positionIndex = -1;
  }
...
const firstLetters = (arr) => {
  const firstLettetArray = arr.map(element => element[0]);
  return firstLettetArray;
  // Solution code here...
};
...
const standardizePhoneNumbers = (arr) => {
  let array = arr.map((element) => {
    let newArray = element.replace(/\D/g,'');
    return newArray;
  });
  return array;
  // Solution code here...
...
const allHappy = (arr) => {
  ...
  const happyArray = arr.filter(element => element.includes(':)'));
