...
const { splitLines } = require("eslint/lib/source-code/source-code");
...
const reverse = (name) => {
  ...
  let reversedName = splitName.reduce((pre, cur) => cur + pre, '');
  return reversedName;
}

const getNames = (arr) => {
  let array = arr.map(Element => reverse(Element['name']));
  return array;
  // Solution code here...
};
...
const setStatusAsAuthor = (people) => {
  people = people.map(element => element['isAuthor'] = true);
  // Solution code here...
};
